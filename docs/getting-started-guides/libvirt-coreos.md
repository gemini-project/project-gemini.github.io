...
The primary goal of the `libvirt-coreos` cluster provider is to deploy a multi-node Gemini cluster on local VMs as fast as possible and to be as light as possible in term of resources used.
...
* Gem API server is reachable via a clear-text connection (no SSL);
* Gem API server requires no credentials;
* etcd access is not protected;
* Gemini secrets are not protected as securely as they are on production environments;
* etc.

So, an gem application developer should not validate its interaction with Gemini on `libvirt-coreos` because he might technically succeed in doing things that are prohibited on a production environment like:

* un-authenticated access to Gem API server;
* Access to Gemini private data structures inside etcd;
* etc.

On the other hand, `libvirt-coreos` might be useful for people investigating low level implementation of Gemini because debugging techniques like sniffing the network traffic or introspecting the etcd content are easier on `libvirt-coreos` than on a production deployment.
...
All the disk drive resources needed by the VM (CoreOS disk image, Gemini binaries, cloud-init files, etc.) are put inside `./cluster/libvirt-coreos/libvirt_storage_pool`.
...
If your `$HOME` is world readable, everything is fine. If your $HOME is private, `cluster/gem-up.sh` will fail with an error message like:

```shell
error: Cannot access storage file '$HOME/.../gemini/cluster/libvirt-coreos/libvirt_storage_pool/gemini_master.img' (as uid:99, gid:78): Permission denied
```
...
By default, the libvirt-coreos setup will create a single Gemini master and 3 Gemini nodes. Because the VM drives use Copy-on-Write and because of memory ballooning and KSM, there is a lot of resource over-allocation.
...
export GEMINI_PROVIDER=libvirt-coreos
cluster/gem-up.sh
```
...
The `GEM_PUSH` environment variable may be set to specify which Gemini binaries must be deployed on the cluster. Its possible values are:

* `release` (default if `GEM_PUSH` is not set) will deploy the binaries of `_output/release-tars/gemini-server-….tar.gz`. This is built with `make release` or `make release-skip-tests`.
...
$ virsh -c qemu:///system list
 Id    Name                           State
----------------------------------------------------
 15    gemini_master              running
 16    gemini_node-01             running
 17    gemini_node-02             running
 18    gemini_node-03             running
 ```

You can check that the Gemini cluster is working with:
...
Connect to `gemini_master`:

```shell
ssh core@192.168.10.1
```

Connect to `gemini_node-01`:

```shell
ssh core@192.168.10.2
```

### Interacting with your Gemini cluster with the `gem-*` scripts.

All of the following commands assume you have set `GEMINI_PROVIDER` appropriately:
...
Update the libvirt-CoreOS cluster with a new Gemini release produced by `make release` or `make release-skip-tests`:

```shell
cluster/gem-push.sh
```

Update the libvirt-CoreOS cluster with the locally built Gemini binaries produced by `make`:

```shell
GEM_PUSH=local cluster/gem-push.sh
```
...
#### !!! Cannot find gemini-server-linux-amd64.tar.gz
