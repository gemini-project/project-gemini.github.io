...
Configuring Gemini on Fedora via Ansible offers a simple way to quickly create a clustered environment with little effort.
...
1. Host able to run ansible and able to clone the following repo: [gemini](https://github.com/gemini-project/gemini.git)
...
A Gemini cluster requires etcd, a master, and n nodes, so we will create a cluster with three hosts, for example:

```shell
master,etcd = gem-master.example.com
    node1 = gem-node-01.example.com
    node2 = gem-node-02.example.com
```
...
**Now clone down the Gemini repository**
...
```shell
[masters]
gem-master.example.com

[etcd]
gem-master.example.com

[nodes]
gem-node-01.example.com
gem-node-02.example.com
```

## Setting up ansible access to your nodes

If you already are running on a machine which has passwordless ssh access to the gem-master and gem-node-{01,02} nodes, and 'sudo' privileges, simply set the value of `ansible_ssh_user` in `~/contrib/ansible/group_vars/all.yml` to the username which you use to ssh to the nodes (i.e. `fedora`), and proceed to the next step...
...
```shell
for node in gem-master.example.com gem-node-01.example.com gem-node-02.example.com; do
  ssh-copy-id ${node}
done
```
...
**Configure access to gemini packages**

Modify `source_type` as below to access gemini packages through the package manager.
...
Each Gemini service gets its own IP address.  These are not real IPs.  You need only select a range of IPs which are not in use elsewhere in your environment.

```yaml
gem_service_addresses: 10.254.0.0/16
```
...
This will finally setup your whole Gemini cluster for you.
...
That's all there is to it.  It's really that easy.  At this point you should have a functioning Gemini cluster.

**Show gemini nodes**

Run the following on the gem-master:
...
```shell
gemctl create -f /tmp/apache.json
```
