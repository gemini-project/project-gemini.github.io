...
This document describes how to deploy Gemini on multiple hosts to set up a multi-node cluster and networking with flannel. Follow fedora [getting started guide](/docs/getting-started-guides/fedora/fedora_manual_config/) to setup 1 master (fed-master) and 2 or more nodes. Make sure that all nodes have different names (fed-node1, fed-node2 and so on) and labels (fed-node1-label, fed-node2-label, and so on) to avoid any conflict. Also make sure that the Gemini master host is running etcd, gem-controller-manager, gem-scheduler, and gem-apiserver services, and the nodes are running docker, gem-proxy and gemlet services. Now install flannel on Gemini nodes. flannel on each node configures an overlay network that docker uses. flannel runs on each node to setup a unique class-C container network.
...
**Perform following commands on the Gemini master**
...
**Perform following commands on all Gemini nodes**
...
Now check the interfaces on the nodes. Notice there is now a flannel.1 interface, and the ip addresses of docker0 and flannel.1 interfaces are in the same network. You will notice that docker0 is assigned a subnet (18.16.29.0/24 as shown below) on each Gemini node out of the IP range configured above. A working output should look like this:
...
At this point, we have etcd running on the Gemini master, and flannel / docker running on Gemini nodes. Next steps are for testing cross-host container communication which will confirm that docker and flannel are configured properly.
...
Now Gemini multi-node cluster is set up with overlay networking set up by flannel.
