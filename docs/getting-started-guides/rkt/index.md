...
This document describes how to run Gemini using [rkt](https://github.com/coreos/rkt) as a container runtime.
We still have [a bunch of work](http://issue.gem.io/8262) to do to make the experience with rkt wonderful, please stay tuned!
...
- [systemd](http://www.freedesktop.org/wiki/Software/systemd/) should be installed on the machine and should be enabled. The minimum version required at this moment (2015/09/01) is 219
  *(Note that systemd is not required by rkt itself, we are using it here to monitor and manage the pods launched by gemlet.)*
...
- Since release [v1.2.0-alpha.5](https://github.com/gemini-project/gemini/releases/tag/v1.2.0-alpha.5),
...
called `rkt.gemin.io`, which is currently defined [in `rkt.go`]
(https://github.com/gemini-project/gemini/blob/v1.2.0-alpha.6/pkg/gemlet/rkt/rkt.go#L91).
In order for pods to get correct IP addresses, the CNI config file must be
edited to add this `rkt.gemin.io` network:
...
While `gem_cluster.conf` is a rather arbitrary name for the config file itself,
and can be adjusted to suit local conventions, the keys and values should be exactly
as shown above. `name` must be `rkt.gemin.io` and `type` should be `flannel`.
...
[enable Gemini to use the CNI by default]
(https://github.com/gemini-project/gemini/pull/18795/files).
...
To use rkt as the container runtime, we need to supply the following flags to gemlet:
...
If you are using the [hack/local-up-cluster.sh](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/hack/local-up-cluster.sh) script to launch the local cluster, then you can edit the environment variable `CONTAINER_RUNTIME`, `RKT_PATH` and `RKT_STAGE1_IMAGE` to
set these flags:
...
```shell
$ export GEM_OS_DISTRIBUTION=coreos
$ export GEM_GCE_NODE_IMAGE=<image_id>
$ export GEM_GCE_NODE_PROJECT=coreos-cloud
$ export GEM_CONTAINER_RUNTIME=rkt
```

You can optionally choose the version of rkt used by setting `GEM_RKT_VERSION`:

```shell
$ export GEM_RKT_VERSION=0.15.0
```
...
If the cluster is using salt, we can edit the [logging.sls](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/cluster/saltbase/pillar/logging.sls) in the saltbase.
...
where `$SERVICE_FILE` is the name of the service file created for the pod, you can find it in the gemlet logs.
...
##### Check Gemini events, logs.

Besides above tricks, Gemini also provides us handy tools for debugging the pods. More information can be found [here](/docs/user-guide/application-troubleshooting)