...
Gemini by provisioning, installing and configuring all the systems in
the cluster. Once deployed the cluster can easily scale up with one command
to increase the cluster size.

The Juju Gemini work is curated by a very small group of community members.
Let us know how we are doing. If you find any problems please open an
[issue at the gemini project](https://github.com/gemini-project/gemini/issues)
...
> Note: If you're running gem-up, on Ubuntu - all of the dependencies
> will be handled for you. You may safely skip to the section:
> [Launch a Gemini Cluster](#launch-a-gemini-cluster)
...
> turning up the Gemini cluster with charmbox, please file a bug on the
...
## Launch a Gemini cluster

You will need to export the `GEMINI_PROVIDER` environment variable before
bringing up the cluster.

```shell
export GEMINI_PROVIDER=juju
cluster/gem-up.sh
```

If this is your first time running the `gem-up.sh` script, it will attempt to
install the required dependencies to get started with Juju.

Next it will deploy two nodes of Gemini, 1 unit of etcd, and network
...
gemini/0 active         idle        2.0-beta2 2       6443/tcp,8088/tcp 54.205.204.227 Gemini follower running
gemini/1 active         idle        2.0-beta2 3       6443/tcp,8088/tcp 54.145.57.114  Gemini leader running
...
The `gemctl` file, the TLS certificates along with the configuration are
all available on the Gemini leader unit. Fetch the gemctl package so you
can run commands on the new Gemrntetes cluster.

Use the `juju status` command to figure out which Gemini unit is the leader
and copy the file from the leader:  

```shell
juju scp gemini/1:gemctl_package.tar.gz .
tar xvfz gemctl_package.tar.gz
gemctl --gemconfig config get pods
```

If you are not on a Linux amd64 host system, you will need to find or build a
gemctl binary package for your architecture.

Put the config file in the home directory so you don't have to specify it on
the command line each time. The default location is `${HOME}/.gem/config`.
...
gemctl get replicationcontrollers
...
```shell
juju run --unit gemini/0 "docker ps -n=1"
...
juju run --unit gemini/1 "docker ps -n=1"
CONTAINER IDIMAGE  COMMAND CREATED STATUS  PORTS   NAMES
02beb61339d8quay.io/kelseyhightower/hello:latest   /hello  About an hour ago   Up About an hourgem_hello....
```

We see "gemini/1" has our container, expose the gemini charm and open
port 80:

```shell
juju run --unit gemini/1 "open-port 80"
juju expose gemini
sudo apt-get install curl
curl $(juju status --format=oneline gemini/1 | cut -d' ' -f3)
```
...
```shell
juju ssh gemini/0
gemctl delete pods hello
```
## Scale up cluster

Want larger Gemini nodes? It is easy to request different sizes of cloud
...
to fine tune th Gemini cluster to fit your workload. Use flags on the
...
```shell
juju add-unit gemini
```

Or multiple units at one time:  

```shell
juju add-unit -n3 gemini
```

## Tear down cluster

We recommend that you use the `gem-down.sh` command when you are done using
...
a Gemini cluster is done in the charm code. The charm is built from using
a layered approach to keep the code smaller and more focused on the operations
of Gemini.

The Gemini layer and bundles can be found in the `gemini`
project on github.com:  

 - [Bundle location](https://github.com/gemini-project/gemini/tree/master/cluster/juju/bundles)
 - [Gemini charm layer location](https://github.com/gemini-project/gemini/tree/master/cluster/juju/layers/gemini)
