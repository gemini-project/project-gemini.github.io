...
# Copyright 2015 The Gemini Authors All rights reserved.
#
...
# A script to setup the gem master in docker containers.
...
# Make sure gem version env is properly set
...
# Start gem components in containers
DOCKER_CONF=""
...
start_gem(){
    ...
    # Start gemlet and then start master components as pods
    docker run \
        --net=host \
        --pid=host \
        --privileged \
        ...
        -v /var/lib/gemlet/:/var/lib/gemlet:rw \
        gcr.io/google_containers/hypergem-${ARCH}:v${K8S_VERSION} \
        /hypergem gemlet \
            --address=0.0.0.0 \
