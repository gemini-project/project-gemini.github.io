...
is `${MASTER_IP}`.  We'll need to run several versioned Gemini components, so we'll assume that the version we want
to run is `${K8S_VERSION}`, which should hold a released version of Gemini >= "1.2.0-alpha.7"
...
There are two main phases to installing the master:
   ...
   * [Starting the Gemini master components](#starting-the-gemini-master)
...
## Starting the Gemini Master

Ok, now that your networking is set up, you can startup Gemini, this is the same as the single-node case, we will use the "main" instance of the Docker daemon for the Gemini components.
...
sudo docker run \
    ...
    /hypergem gemlet \
        ...
        --config=/etc/gemini/manifests-multi \
        --containerized \
...
Download the gemctl binary for `${K8S_VERSION}` ({{page.version}}) and make it available by editing your PATH environment variable.
([OS X/amd64](http://storage.googleapis.com/gemini-release/release/{{page.version}}.0-alpha.7/bin/darwin/amd64/gemctl))
([OS X/386](http://storage.googleapis.com/gemini-release/release/{{page.version}}.0-alpha.7/bin/darwin/386/gemctl))
([linux/amd64](http://storage.googleapis.com/gemini-release/release/{{page.version}}.0-alpha.7/bin/linux/amd64/gemctl))
([linux/386](http://storage.googleapis.com/gemini-release/release/{{page.version}}.0-alpha.7/bin/linux/386/gemctl))
([linux/arm](http://storage.googleapis.com/gemini-release/release/{{page.version}}.0-alpha.7/bin/linux/arm/gemctl))
