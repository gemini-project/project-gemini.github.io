...
Use the [master.yaml](/docs/getting-started-guides/coreos/cloud-configs/master.yaml) and [node.yaml](/docs/getting-started-guides/coreos/cloud-configs/node.yaml) cloud-configs to provision a multi-node Gemini cluster.
...
```shell
aws ec2 create-security-group --group-name gemini --description "Gemini Security Group"
aws ec2 authorize-security-group-ingress --group-name gemini --protocol tcp --port 22 --cidr 0.0.0.0/0
aws ec2 authorize-security-group-ingress --group-name gemini --protocol tcp --port 80 --cidr 0.0.0.0/0
aws ec2 authorize-security-group-ingress --group-name gemini --source-security-group-name gemini
```
...
Next, setup an ssh tunnel to the master so you can run gemctl from your local host.
...
```shell
nova secgroup-create gemini "Gemini Security Group"
nova secgroup-add-rule gemini tcp 22 22   0.0.0.0/0
nova secgroup-add-rule gemini tcp 80 80   0.0.0.0/0
```
...
```shell
nova floating-ip-associate gem-master <ip address>
```
...
Edit `node.yaml` and replace all instances of `<master-private-ip>` with the private IP address of the master node.  You can get this by running `nova show gem-master` assuming you named your instance gem master.  This is not the floating IP address you just assigned it.
