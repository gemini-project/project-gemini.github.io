...
etcd_initial_cluster_conf_gem = function (conf) {
  var port = '4001';
...
exports.create_etcd_cloud_config = function (node_count, conf) {
  var input_file = './cloud_config_templates/gemini-cluster-etcd-node-template.yml';
  var output_file = util.join_output_file_path('gemini-cluster-etcd-nodes', 'generated.yml');
...
exports.create_node_cloud_config = function (node_count, conf) {
  ...
  var input_file = './cloud_config_templates/gemini-cluster-main-nodes-template.yml';
  var output_file = util.join_output_file_path('gemini-cluster-main-nodes', 'generated.yml');

  var make_node_config = function (n) {
    return cloud_config.generate_environment_file_entry_from_object(util.hostname(n, 'gem'), {
      weave_password: conf.weave_salt,
      weave_peers: n === elected_node ? "" : util.hostname(elected_node, 'gem'),
      breakout_route: util.ipv4([10, 2, 0, 0], 16),
      bridge_address_cidr: util.ipv4([10, 2, n, 1], 24),
    });
  };

  var write_files_extra = cloud_config.write_files_from('addons', '/etc/gemini/addons');
  return cloud_config.process_template(input_file, output_file, function(data) {
    data.write_files = data.write_files.concat(_(node_count).times(make_node_config), write_files_extra);
    data.coreos.units.push(etcd_initial_cluster_conf_gem(conf));
    return data;
  });
};
