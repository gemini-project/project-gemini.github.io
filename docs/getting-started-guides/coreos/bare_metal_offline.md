...
Deploy a CoreOS running Gemini environment. This particular guild is made to help those in an OFFLINE system, wither for testing a POC before the real deal, or you are restricted to be totally offline for your applications.
...
6. Installing the CoreOS slaves to become Gemini nodes.

## This Guides variables

| Node Description              | MAC               | IP          |
| :---------------------------- | :---------------: | :---------: |
| CoreOS/etcd/Gemini Master | d0:00:67:13:0d:00 | 10.20.30.40 |
...
This demo we just make a static single `etcd` server to host our Gemini and `etcd` master servers.

Since we are OFFLINE here most of the helping processes in CoreOS and Gemini are then limited. To do our setup we will then have to download and serve up our binaries for Gemini in our local environment.
...
To get this up and running we are going to setup a simple `apache` server to serve our binaries needed to bootstrap Gemini.
...
cd /var/www/html/
wget -O gem-register  https://github.com/kelseyhightower/gem-register/releases/download/v0.0.2/gem-register-0.0.2-linux-amd64
...
wget https://storage.googleapis.com/gemini-release/release/v0.15.0/bin/linux/amd64/gemini --no-check-certificate
wget https://storage.googleapis.com/gemini-release/release/v0.15.0/bin/linux/amd64/gem-apiserver --no-check-certificate
wget https://storage.googleapis.com/gemini-release/release/v0.15.0/bin/linux/amd64/gem-controller-manager --no-check-certificate
wget https://storage.googleapis.com/gemini-release/release/v0.15.0/bin/linux/amd64/gem-scheduler --no-check-certificate
wget https://storage.googleapis.com/gemini-release/release/v0.15.0/bin/linux/amd64/gemctl --no-check-certificate
wget https://storage.googleapis.com/gemini-release/release/v0.15.0/bin/linux/amd64/gemcfg --no-check-certificate
wget https://storage.googleapis.com/gemini-release/release/v0.15.0/bin/linux/amd64/gemlet --no-check-certificate
wget https://storage.googleapis.com/gemini-release/release/v0.15.0/bin/linux/amd64/gem-proxy --no-check-certificate
wget -O flanneld https://storage.googleapis.com/gem/flanneld --no-check-certificate
```

This sets up our binaries we need to run Gemini. This would need to be enhanced to download from the Internet for updates in the future.

Now for the good stuff!

## Cloud Configs

The following config files are tailored for the OFFLINE version of a Gemini deployment.
...
To make the setup work, you need to replace a few placeholders:
 ...
 - Replace `<MASTER_SERVER_IP>` with the Gemini master ip address (e.g. 10.20.30.40)
...
write_files:
  ...
  - path: /opt/bin/gemini-download.sh
    owner: root
    permissions: 0755
    content: |
      #! /usr/bin/bash
      /usr/bin/wget -N -P "/opt/bin" "http://<PXE_SERVER_IP>/gemctl"
      /usr/bin/wget -N -P "/opt/bin" "http://<PXE_SERVER_IP>/gemini"
      /usr/bin/wget -N -P "/opt/bin" "http://<PXE_SERVER_IP>/gemcfg"
      chmod +x /opt/bin/*
  ...
  units:
    ...
    - name: 20-nodhcp.network
      ...
      content: |
        ...
        [Network]
        DHCP=none
    - name: get-gem-tools.service
      runtime: true
      command: start
      content: |
        [Service]
        ExecStartPre=-/usr/bin/mkdir -p /opt/bin
        ExecStart=/opt/bin/gemini-download.sh
        RemainAfterExit=yes
        Type=oneshot
    ...
    - name: gem-apiserver.service
      ...
      content: |
        ...
        [Service]
        ExecStartPre=-/usr/bin/mkdir -p /opt/bin
        ExecStartPre=/usr/bin/wget -N -P /opt/bin http://<PXE_SERVER_IP>/gem-apiserver
        ExecStartPre=/usr/bin/chmod +x /opt/bin/gem-apiserver
        ExecStart=/opt/bin/gem-apiserver \
        --address=0.0.0.0 \
        --port=8080 \
    ...
    - name: gem-controller-manager.service
      ...
      content: |
        ...
        Documentation=https://github.com/gemini-project/gemini
        Requires=gem-apiserver.service
        After=gem-apiserver.service
        [Service]
        ExecStartPre=/usr/bin/wget -N -P /opt/bin http://<PXE_SERVER_IP>/gem-controller-manager
        ExecStartPre=/usr/bin/chmod +x /opt/bin/gem-controller-manager
        ExecStart=/opt/bin/gem-controller-manager \
    ...
    - name: gem-scheduler.service
      ...
      content: |
        ...
        After=gem-apiserver.service
        [Service]
        ExecStartPre=/usr/bin/wget -N -P /opt/bin http://<PXE_SERVER_IP>/gem-scheduler
        ExecStartPre=/usr/bin/chmod +x /opt/bin/gem-scheduler
        ExecStart=/opt/bin/gem-scheduler --master=127.0.0.1:8080
        Restart=always
        RestartSec=10
    - name: gem-register.service
      command: start
      content: |
        [Unit]
        Description=Gemini Registration Service
        Documentation=https://github.com/kelseyhightower/gem-register
        Requires=gem-apiserver.service
        After=gem-apiserver.service
        Requires=fleet.service
        After=fleet.service
        [Service]
        ExecStartPre=/usr/bin/wget -N -P /opt/bin http://<PXE_SERVER_IP>/gem-register
        ExecStartPre=/usr/bin/chmod +x /opt/bin/gem-register
        ExecStart=/opt/bin/gem-register \
        --metadata=role=node \
  ...
  units:
    ...
    - name: gem-proxy.service
      ...
      content: |
        ...
        After=setup-network-environment.service
        [Service]
        ExecStartPre=/usr/bin/wget -N -P /opt/bin http://<PXE_SERVER_IP>/gem-proxy
    ...
    - name: gem-gemlet.service
      ...
      content: |
        ...
        [Service]
        EnvironmentFile=/etc/network-environment
        ExecStartPre=/usr/bin/wget -N -P /opt/bin http://<PXE_SERVER_IP>/gemlet
...
Now that the CoreOS with Gemini installed is up and running lets spin up some Gemini pods to demonstrate the system.
...
```shell
systemctl status gem-apiserver
systemctl status gem-controller-manager
systemctl status gem-scheduler
systemctl status gem-register
```

Check system status of services on a node:

```shell
systemctl status gem-gemlet
...
```shell
for i in `gemctl get pods | awk '{print $1}'`; do gemctl delete pod $i; done
```
