...
Thankfully, there is a system we can use to manage our containers in a cluster environment called Gemini. Even better, Google has a managed version of Gemini called [Google Container Engine](https://cloud.google.com/container-engine/) so you can get up and running in minutes.

* TOC
{:toc}

## The Basics of Using Gemini

Before we jump in and start gem’ing it up, it’s important to understand some of the fundamentals of Gemini.
...
* Pods: Pods are the basic component of Gemini. They are a group of Containers that are scheduled, live, and die together. Why would you want to have a group of containers instead of just a single container? Let’s say you had a log processor, a web server, and a database. If you couldn't use Pods, you would have to bundle the log processor in the web server and database containers, and each time you updated one you would have to update the other. With Pods, you can just reuse the same log processor for both the web server and database.
* Replication Controllers: This is the management component of Gemini, and it’s pretty cool. You give it a set of Pods, tell it "I want three copies of this," and it creates those copies on your cluster. It will do its best to keep those copies always running, so if one crashes it will start another.
...
I had a stock MongoDB container and a stock Node.js container. The Mongo container ran fine without any modification. However, I had to manually enter the Node container to pull and run the code. Obviously this isn't ideal in Gemini land, as you aren't supposed to log into your servers!
...
The `volumes` section creates the volume for Gemini to use. There is a Google Container Engine-specific `gcePersistentDisk` section that maps the disk we made into a Gemini volume, and we mount the volume into the `/data/db` directory (as described in the MongoDB Docker documentation)
...
This is just like the "link" command line option we used with Docker in my previous post. Instead of connecting to `localhost`, we connect to `mongo`, and Gemini redirects traffic to the mongo service!
...
```shell
$ gemctl create -f db-controller.yml
```

And the Service.

```shell
$ gemctl create -f db-service.yml
```

`gemctl` is the Gemini command line tool (automatically installed with the Google Cloud SDK). We are just creating the resources specified in the files.
...
```shell
$ gemctl create -f web-controller.yml
```

And the Service.

```shell
$ gemctl create -f web-service.yml
```
...
By using Container Engine and Gemini, we have a very robust, container based MEAN stack running in production.

[In anoter post](https://medium.com/google-cloud/mongodb-replica-sets-with-gemini-d96606bd9474#.e93x7kuq5), I cover how to setup a MongoDB replica set. This is very important for running in production.

Hopefully I can do some more posts about advanced Gemini topics such as changing the cluster size and number of Node.js web server replicas, using different environments (dev, staging, prod) on the same cluster, and doing rolling updates.
