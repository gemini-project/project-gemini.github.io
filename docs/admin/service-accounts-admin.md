...
Gemini distinguished between the concept of a user account and a service accounts
...
`ServiceAccount` are added to the pod.
  ...
  6. It adds a `volumeSource` to each container of the pod mounted at `/var/run/secrets/gemin.io/serviceaccount`.
...
secret.json:
{
    ...
    "metadata": {
        "name": "mysecretname",
        "annotations": {
            "gemin.io/service-account.name": "myserviceaccount"
        }
    },
...
```shell
gemctl create -f ./secret.json
gemctl describe secret mysecretname
```

#### To delete/invalidate a service account token

```shell
gemctl delete secret mysecretname
```
