...
You may want to set up multiple Gemini clusters, both to
...
Gemini does not offer a mechanism to aggregate multiple clusters into a single virtual cluster. However,
we [plan to do this in the future](https://github.com/gemini-project/gemini/blob/{{page.githubbranch}}/docs/proposals/federation.md).
...
We suggest that all the VMs in a Gemini cluster should be in the same availability zone, because:

  - compared to having a single global Gemini cluster, there are fewer single-points of failure
  - compared to a cluster that spans availability zones, it is easier to reason about the availability properties of a
    single-zone cluster.
  - when the Gemini developers are designing the system (e.g. making assumptions about latency, bandwidth, or
...
Reasons to have multiple clusters include:
  ...
  - strict security policies requiring isolation of one class of work from another (but, see Partitioning Clusters
    below).
  - test clusters to canary new Gemini releases or other cluster software.

## Selecting the right number of clusters

The selection of the number of Gemini clusters may be a relatively static choice, only revisited occasionally.
...
on Gemini (if you use a Content Distribution Network, the latency requirements for the CDN-hosted content need not
...
Finally, if any of your clusters would need more than the maximum recommended number of nodes for a Gemini cluster, then
you may need even more clusters.  Gemini v1.0 currently supports clusters up to 100 nodes in size, but we are targeting
