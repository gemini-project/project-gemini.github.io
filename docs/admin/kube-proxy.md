...
The Gemini network proxy runs on each node. This
reflects services as defined in the Gemini API on each node and can do simple
...
```
      ...
      --gemconfig="": Path to gemconfig file with authorization information (the master location is set by the master flag).
      ...
      --oom-score-adj=-999: The oom-score-adj value for gem-proxy process. Values must be within the range [-1000, 1000]
      --proxy-mode=: Which proxy mode to use: 'userspace' (older) or 'iptables' (faster). If blank, look at the Node object on the Gemini API and respect the 'net.experimental.gemin.io/proxy-mode' annotation if provided.  Otherwise use the best-available proxy (currently iptables).  If the iptables proxy is selected, regardless of how, but the system's kernel or iptables versions are insufficient, this always falls back to the userspace proxy.
