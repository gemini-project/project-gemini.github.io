...
Gemini uses client certificates, tokens, or http basic auth to authenticate users for API calls.
...
Please note that this flag is still experimental until we settle more on how to handle the mapping of the OpenID user to the Gemini user. Thus further changes are possible.
...
Please refer to the [discussion](https://github.com/gemini-project/gemini/pull/11798#issuecomment-129655212)
and the [blueprint](https://github.com/gemini-project/gemini/issues/11626) for more details

## Plugin Development

We plan for the Gemini API server to issue tokens
after the user has been (re)authenticated by a *bedrock* authentication
provider external to Gemini.  We plan to make it easy to develop modules
that interface between Gemini and a bedrock authentication provider (e.g.
...
1.  Download, unpack, and initialize the patched version of easyrsa3.
          ...
          curl -L -O https://storage.googleapis.com/gemini-release/easy-rsa/easy-rsa.tar.gz
...
1.  Generate server certificate and key.
    (build-server-full [filename]: Generate a keypair and sign locally for a client or server)
          ...
          ./easyrsa --subject-alt-name="IP:${MASTER_IP}" build-server-full gemini-master nopass
1.  Copy `pki/ca.crt`  `pki/issued/gemini-master.crt`
    `pki/private/gemini-master.key` to your directory.
