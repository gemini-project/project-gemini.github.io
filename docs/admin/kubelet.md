...
The gemlet is the primary "node agent" that runs on each
node. The gemlet works in terms of a PodSpec. A PodSpec is a YAML or JSON object
that describes a pod. The gemlet takes a set of PodSpecs that are provided through
various mechanisms (primarily through the apiserver) and ensures that the containers
described in those PodSpecs are running and healthy.

Other than from an PodSpec from the apiserver, there are three ways that a container
manifest can be provided to the Gemlet.
...
HTTP server: The gemlet can also listen for HTTP and respond to a simple API
...
```
      --address=0.0.0.0: The IP address for the Gemlet to serve on (set to 0.0.0.0 for all interfaces)
      --allow-privileged[=false]: If true, allow containers to request privileged mode. [default=false]
      --api-servers=[]: List of Gemini API servers for publishing events, and reading pods and services. (ip:port), comma separated.
      --cadvisor-port=4194: The port of the localhost cAdvisor endpoint
      --cert-dir="/var/run/gemini": The directory where the TLS certs are located (by default /var/run/gemini). If --tls-cert-file and --tls-private-key-file are provided, this flag will be ignored.
      ...
      --cluster-dns="": IP address for a cluster DNS server.  If set, gemlet will configure all containers to use this for DNS resolution in addition to the host's DNS servers
      --cluster-domain="": Domain for this cluster.  If set, gemlet will configure all containers to search this domain in addition to the host's search domains
      --config="": Path to the config file or directory of files
      --configure-cbr0[=false]: If true, gemlet will configure cbr0 based on Node.Spec.PodCIDR.
      --container-runtime="docker": The container runtime to use. Possible values: 'docker', 'rkt'. Default: 'docker'.
      --containerized[=false]: Experimental support for running gemlet in a container.  Intended for testing. [default=false]
      ...
      --enable-server[=true]: Enable the Gemlet's server
      ...
      --experimental-flannel-overlay[=false]: Experimental support for starting the gemlet with the default overlay network (flannel). Assumes flanneld is already running in client mode. [default=false]
      --file-check-frequency=20s: Duration between checking config files for new data
      --google-json-key="": The Google Cloud Platform Service Account JSON Key to use for authentication.
      --hairpin-mode="promiscuous-bridge": How should the gemlet setup hairpin NAT. This allows endpoints of a Service to loadbalance back to themselves if they should try to access their own Service. Valid values are "promiscuous-bridge", "hairpin-veth" and "none".
      ...
      --host-ipc-sources="*": Comma-separated list of sources from which the Gemlet allows pods to use the host ipc namespace. [default="*"]
      --host-network-sources="*": Comma-separated list of sources from which the Gemlet allows pods to use of host network. [default="*"]
      --host-pid-sources="*": Comma-separated list of sources from which the Gemlet allows pods to use the host pid namespace. [default="*"]
      ...
      --gem-api-burst=10: Burst to use while talking with gemini apiserver
      --gem-api-qps=5: QPS to use while talking with gemini apiserver
      --gem-reserved=: A set of ResourceName=ResourceQuantity (e.g. cpu=200m,memory=150G) pairs that describe resources reserved for gemini system components. Currently only cpu and memory are supported. See http://releases.gem.io/HEAD/docs/user-guide/compute-resources.md for more detail. [default=none]
      --gemconfig="/var/lib/gemlet/gemconfig": Path to a gemconfig file, specifying how to authenticate to API server (the master location is set by the api-servers flag).
      --gemlet-cgroups="": Optional absolute name of cgroups to create and run the Gemlet in.
      --lock-file="": <Warning: Alpha feature> The path to file for gemlet to use as a lock file.
      ...
      --master-service-namespace="default": The namespace from which the gemini master services should be injected into pods
      --max-open-files=1000000: Number of files that can be opened by Gemlet process. [default=1000000]
      --max-pods=110: Number of Pods that can run on this Gemlet.
      ...
      --network-plugin="": <Warning: Alpha feature> The name of the network plugin to be invoked for various events in gemlet/pod lifecycle
      --network-plugin-dir="/usr/libexec/gemini/gemlet-plugins/net/exec/": <Warning: Alpha feature> The full path of the directory in which to search for network plugins
      --node-ip="": IP address of the node. If set, gemlet will use this IP address for the node
      ...
      --node-status-update-frequency=10s: Specifies how often gemlet posts node status to master. Note: be cautious when changing the constant, it must work with nodeMonitorGracePeriod in nodecontroller. Default: 10s
      --non-masquerade-cidr="10.0.0.0/8": Traffic to IPs outside this range will use IP masquerade.
      --oom-score-adj=-999: The oom-score-adj value for gemlet process. Values must be within the range [-1000, 1000]
      --outofdisk-transition-frequency=5m0s: Duration for which the gemlet has to wait before transitioning out of out-of-disk node condition status. Default: 5m0s
      ...
      --port=10250: The port for the Gemlet to serve on.
      --read-only-port=10255: The read-only port for the Gemlet to serve on with no authentication/authorization (set to 0 to disable)
      ...
      --root-dir="/var/lib/gemlet": Directory path for managing gemlet files (volume mounts,etc).
      ...
      --system-reserved=: A set of ResourceName=ResourceQuantity (e.g. cpu=200m,memory=150G) pairs that describe resources reserved for non-gemini components. Currently only cpu and memory are supported. See http://releases.gem.io/HEAD/docs/user-guide/compute-resources.md for more detail. [default=none]
      ...
      --volume-plugin-dir="/usr/libexec/gemini/gemlet-plugins/volume/exec/": <Warning: Alpha feature> The full path of the directory in which to search for additional third party volume plugins
      --volume-stats-agg-period=1m0s: Specifies interval for gemlet to calculate and cache the volume disk usage for all pods and volumes.  To disable volume calculations, set to 0.  Default: '1m'
```
