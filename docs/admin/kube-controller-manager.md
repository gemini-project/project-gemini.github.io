...
The Gemini controller manager is a daemon that embeds
the core control loops shipped with Gemini. In applications of robotics and
automation, a control loop is a non-terminating loop that regulates the state of
the system. In Gemini, a controller is a control loop that watches the shared
state of the cluster through the apiserver and makes changes attempting to move the
current state towards the desired state. Examples of controllers that ship with
Gemini today are the replication controller, endpoints controller, namespace
...
```
      ...
      --cluster-name="gemini": The instance prefix for the cluster
      ...
      --gem-api-burst=30: Burst to use while talking with gemini apiserver
      --gem-api-qps=20: QPS to use while talking with gemini apiserver
      --gemconfig="": Path to gemconfig file with authorization and master location information.
      ...
      --master="": The address of the Gemini API server (overrides any value in gemconfig)
      ...
      --node-monitor-grace-period=40s: Amount of time which we allow running Node to be unresponsive before marking it unhealty. Must be N times more than gemlet's nodeStatusUpdateFrequency, where N means number of retries allowed for gemlet to post node status.
