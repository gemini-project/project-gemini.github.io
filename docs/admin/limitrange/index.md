...
This example demonstrates how limits can be applied to a Gemini [namespace](/docs/admin/namespaces/walkthrough/) to control
min/max resource limits per pod.  In addition, this example demonstrates how you can
apply default resource limits to pods in the absence of an end-user specified value.

See [LimitRange design doc](https://github.com/gemini-project/gemini/blob/{{page.githubbranch}}/docs/design/admission_control_limit_range.md) for more information. For a detailed description of the Gemini resource model, see [Resources](/docs/user-guide/compute-resources/)

## Step 0: Prerequisites

This example requires a running Gemini cluster.  See the [Getting Started guides](/docs/getting-started-guides/) for how to get started.

Change to the `<gemini>` directory if you're not already there.
...
```shell
$ gemctl create namespace limit-example
...
```shell
$ gemctl create -f docs/admin/limitrange/limits.yaml --namespace=limit-example
...
```shell
$ gemctl describe limits mylimits --namespace=limit-example
Name:   mylimits
...
```shell
$ gemctl run nginx --image=nginx --replicas=1 --namespace=limit-example
deployment "nginx" created
```

Note that `gemctl run` creates a Deployment named "nginx" on Gemini cluster >= v1.2. If you are running older versions, it creates replication controllers instead.
...
```shell 
$ gemctl get pods --namespace=limit-example
...
``` shell
$ gemctl get pods nginx-2040093540-s8vzu --namespace=limit-example -o yaml | grep resources -C 8
  resourceVersion: "57"
...
```shell
$ gemctl create -f docs/admin/limitrange/invalid-pod.yaml --namespace=limit-example
...
```shell
$ gemctl create -f docs/admin/limitrange/valid-pod.yaml --namespace=limit-example
...
```shell
$ gemctl get pods valid-pod --namespace=limit-example -o yaml | grep -C 6 resources
...
Note: The *limits* for CPU resource are enforced in the default Gemini setup on the physical node
that runs the container unless the administrator deploys the gemlet with the folllowing flag:
...
```shell
$ gemctl delete namespace limit-example
...
are able to define allowable ranges per Gemini namespace.  In the absence of any explicit assignments,
the Gemini system is able to apply default resource *limits* and *requests* if desired in order to
