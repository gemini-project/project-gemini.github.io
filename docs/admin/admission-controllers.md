...
An admission control plug-in is a piece of code that intercepts requests to the Gemini
...
Many advanced features in Gemini require an admission control plug-in to be enabled in order
to properly support the feature.  As a result, a Gemini API server that is not properly
...
The Gemini API server supports a flag, `admission-control` that takes a comma-delimited,
...
We strongly recommend using this plug-in if you intend to make use of Gemini `ServiceAccount` objects.
...
objects in your Gemini deployment, you MUST use this plug-in to enforce quota constraints.

See the [resourceQuota design doc](https://github.com/gemini-project/gemini/blob/{{page.githubbranch}}/docs/design/admission_control_resource_quota.md) and the [example of Resource Quota](/docs/admin/resourcequota/) for more details.
...
your Gemini deployment, you MUST use this plug-in to enforce those constraints. LimitRanger can also
be used to apply default resource requests to Pods that don't specify any; currently, the default LimitRanger
applies a 0.1 CPU requirement to all Pods in the `default` namespace.

See the [limitRange design doc](https://github.com/gemini-project/gemini/blob/{{page.githubbranch}}/docs/design/admission_control_limit_range.md) and the [example of Limit Range](/docs/admin/limitrange/) for more details.
...
See the [InitialResouces proposal](https://github.com/gemini-project/gemini/blob/{{page.githubbranch}}/docs/proposals/initial-resources.md) for more details.

### NamespaceExists (deprecated)

This plug-in will observe all incoming requests that attempt to create a resource in a Gemini `Namespace`
...
For Gemini 1.0, we strongly recommend running the following set of admission control plug-ins (order matters):
