...
store which Gemini uses for persistent storage of all of its REST API
objects.

## Configuration: high-level goals

Access Control: give *only* gem-apiserver read/write access to etcd. You do not
...
The default setup scripts use gemlet's file-based static pods feature to run etcd in a
[pod](http://releases.gem.io/{{page.githubbranch}}/cluster/saltbase/salt/etcd/etcd.manifest). This manifest should only
be run on master VMs. The default location that gemlet scans for manifests is
`/etc/gemini/manifests/`.

## Gemini's usage of etcd

By default, Gemini objects are stored under the `/registry` key in etcd.
This path can be prefixed by using the [gem-apiserver](/docs/admin/gem-apiserver) flag
`--etcd-prefix="/foo"`.

`etcd` is the only place that Gemini keeps state.
