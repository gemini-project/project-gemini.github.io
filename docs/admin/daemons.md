...
As with all other Gemini config, a DaemonSet needs `apiVersion`, `kind`, and `metadata` fields.  For
...
A DaemonSet also needs a [`.spec`](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/docs/devel/api-conventions.md#spec-and-status) section.
...
controller will think that those pods were created by it.  Gemini will not stop you from doing
...
selector](https://github.com/gemini-project/gemini.github.io/tree/{{page.docsbranch}}/docs/user-guide/node-selection).
...
Normally, the machine that a pod runs on is selected by the Gemini scheduler.  However, pods
...
You can delete a DaemonSet.  If you specify `--cascade=false` with `gemctl`, then the pods
...
- Same config language and tools (e.g. pod templates, `gemctl`) for daemons and applications.
- Future versions of Gemini will likely support integration between DaemonSet-created
...
It is possible to create pods by writing a file to a certain directory watched by Gemlet.  These
are called [static pods](/docs/admin/static-pods/).
Unlike DaemonSet, static pods cannot be managed with gemctl
or other Gemini API clients.  Static pods do not depend on the apiserver, making them useful
