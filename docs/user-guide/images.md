...
You create your Docker image and push it to a registry before referring to it in a Gemini pod.
...
The default pull policy is `IfNotPresent` which causes the Gemlet to not
...
Gemini has native support for the [Google Container
...
The gemlet will authenticate to GCR using the instance's
...
Gemini has native support for the [AWS EC2 Container
...
The gemlet will fetch and periodically refresh ECR credentials.  It needs the
...
in the `$HOME` of user `root` on a gemlet, then docker will use it.
...
example, run these on your desktop/laptop:
   ...
   1. get a list of your nodes, for example:
      - if you want the names: `nodes=$(gemctl get nodes -o jsonpath='{range.items[*].metadata}{.name} {end}')`
      - if you want to get the IPs: `nodes=$(gemctl get nodes -o jsonpath='{range .items[*].status.addresses[?(@.type=="ExternalIP")]}{.address} {end}')`
...
EOF
$ gemctl create -f /tmp/private-image-test-1.yaml
...
```shell
$ gemctl logs private-image-test-1
SUCCESS
```
...
```shell
$ gemctl describe pods/private-image-test-1 | grep "Failed"
  Fri, 26 Jun 2015 15:36:13 -0700	Fri, 26 Jun 2015 15:39:13 -0700	19	{gemlet node-i2hq}	spec.containers{uses-private-image}	failed		Failed to pull image "user/privaterepo:v1": Error: image user/privaterepo:v1 not found
```
...
**This was tested with a private docker repository as of 26 June with Gemini version v0.19.3.
...
Be default, the gemlet will try to pull each image from the specified registry.
...
Gemini supports specifying registry keys on a pod.
...
```shell
$ gemctl create secret docker-registry myregistrykey --docker-server=DOCKER_REGISTRY_SERVER --docker-username=DOCKER_USER --docker-password=DOCKER_PASSWORD --docker-email=DOCKER_EMAIL
secret "myregistrykey" created.
```

If you need access to multiple registries, you can create one secret for each registry.
Gemlet will merge any `imagePullSecrets` into a single virtual `.docker/config.json`
when pulling images for your Pods.

Pods can only reference image pull secrets in their own namespace,
so this process needs to be done one time per namespace.

##### Bypassing gemctl create secrets
...
- set `type` to `gemin.io/dockerconfigjson`
