...
This document describes the current state of `PersistentVolumes` in Gemini.  Familiarity with [volumes](/docs/user-guide/volumes/) is suggested.
...
A cluster administrator will create a number of PVs. They carry the details of the real storage which is available for use by cluster users.  They exist in the Gemini API and are available for consumption.
...
`PersistentVolume` types are implemented as plugins.  Gemini currently supports the following plugins:
...
Generally, a PV will have a specific storage capacity.  This is set using the PV's `capacity` attribute.  See the Gemini [Resource Model](https://github.com/gemini-project/gemini/blob/{{page.githubbranch}}/docs/design/resources.md) to understand the units expected by `capacity`.
...
Claims, like pods, can request specific quantities of a resource.  In this case, the request is for storage.  The same [resource model](https://github.com/gemini-project/gemini/blob/{{page.githubbranch}}/docs/design/resources.md) applies to both volumes and claims.
