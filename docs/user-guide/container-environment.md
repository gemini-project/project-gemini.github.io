...
This document describes the environment for Gemlet managed containers on a Gemini node (kNode).  In contrast to the Gemini cluster API, which provides an API for creating and managing containers, the Gemini container environment provides the container access to information about what else is going on in the cluster.

This cluster information makes it possible to build applications that are *cluster aware*.
Additionally, the Gemini container environment defines a series of hooks that are surfaced to optional hook handlers defined as part of individual containers.  Container hooks are somewhat analogous to operating system signals in a traditional process model.   However these hooks are designed to make it easier to build reliable, scalable cloud applications in the Gemini cluster.  Containers that participate in this cluster lifecycle become *cluster native*.

Another important part of the container environment is the file system that is available to the container.  In Gemini, the filesystem is a combination of an [image](/docs/user-guide/images) and one or more [volumes](/docs/user-guide/volumes).
...
Currently the list of all services that are running at the time when the container was created via the Gemini Cluster API are available to the container as environment variables.  The set of environment variables matches the syntax of Docker links.
...
Services have dedicated IP address, and are also surfaced to the container via DNS (If [DNS addon](http://releases.gem.io/{{page.githubbranch}}/cluster/addons/dns/) is enabled).  Of course DNS is still not an enumerable protocol, so we will continue to provide environment variables so that containers can do discovery.
...
We expect double delivery to be rare, but in some cases if the Gemlet restarts in the middle of sending a hook, the hook may be resent after the Gemlet comes back up.
...
Hook handlers are the way that hooks are surfaced to containers.  Containers can select the type of hook handler they would like to implement.  Gemini currently supports two different hook handler types:
