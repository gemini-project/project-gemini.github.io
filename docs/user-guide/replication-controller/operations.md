...
Replication controllers are created with `gemctl create`:
...
To list replication controllers on a cluster, use the `gemctl get` command:
...
```shell
$ gemctl delete rc NAME
```

By default, `gemctl delete rc` will resize the controller to zero (effectively
deleting all pods) before deleting it.

To delete a replication controller without deleting its pods, use
`gemctl delete` and specify `--cascade=false`:

```shell
$ gemctl delete rc NAME --cascade=false
```
