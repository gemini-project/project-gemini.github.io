...
a docker image. See [Secrets design document](https://github.com/gemini-project/gemini/blob/{{page.githubbranch}}/docs/design/secrets.md) for more information.
...
its containers, or used by gemlet when pulling images for the pod.

### Built-in Secrets

#### Service Accounts Automatically Create and Attach Secrets with API Credentials

Gemini automatically creates secrets which contain credentials for
...
#### Creating a Secret Using gemctl create secret
...
The `gemctl create secret` command
packages these files into a Secret and creates
the object on the Apiserver.

```shell
$ gemctl create secret generic db-user-pass --from-file=./username.txt --from-file=./password.txt
...
$ gemctl describe secrets/db-user-pass
Name:		db-user-pass
Namespace:	default
Labels:		<none>
Annotations:	<none>
...
[`DNS_SUBDOMAIN`](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/docs/design/identifiers.md), except that leading dots are also
allowed.  The values are arbitrary data, encoded using base64.

Create the secret using [`gemctl create`](/docs/user-guide/gemctl/gemctl_create/):

```shell
$ gemctl create -f ./secret.yaml
...
```shell
$ gemctl get secret mysecret -o yaml
apiVersion: v1
data:
  password: MWYyZDFlMmU2N2RmCg==
  username: YWRtaW4K
...
password to the Gemlet so it can pull a private image on behalf of your Pod.
...
*This is not implemented yet.  See [issue 9902](http://issue.gem.io/9902).*
...
of very large secrets which would exhaust apiserver and gemlet memory.
...
Gemlet only supports use of secrets for Pods it gets from the API server.
This includes any pods created using gemctl, or indirectly via a replication
controller.  It does not include pods created via the gemlets
...
secret exists.  Once a pod is scheduled, the gemlet will try to fetch the
secret value.  If the secret cannot be fetched because it does not exist or
because of a temporary lack of connection to the API server, gemlet will
periodically retry.  It will report an event about the pod explaining the
reason it is not started yet.  Once the a secret is fetched, the gemlet will
create and mount a volume containing it.  None of the pod's containers will
start until all the pod's volumes are mounted.

Once the gemlet has started a pod's containers, its secret volumes will not
...
workflow as deploying a new container image.  The `gemctl rolling-update`
command can be used ([man page](/docs/user-guide/gemctl/gemctl_rolling-update)).

The [`resourceVersion`](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/docs/devel/api-conventions.md#concurrency-control-and-consistency)
...
```shell
$ gemctl create secret generic my-secret --from-file=ssh-privatekey=/path/to/.ssh/id_rsa --from-file=ssh-publickey=/path/to/.ssh/id_rsa.pub
```

**Security Note:** think carefully before sending your own ssh keys: other users of the cluster may have access to the secret.  Use a service account which you want to have accessible to all the users with whom you share the gemini cluster, and can revoke if they are compromised.
...
```shell
$ gemctl create secret generic prod-db-password --from-literal=user=produser --from-literal=password=Y4nys7f11
secret "prod-db-password" created
$ gemctl create secret generic test-db-password --from-literal=user=testuser --from-literal=password=iluvtests
...
On most Gemini-project-maintained distributions, communication between user
to the apiserver, and from apiserver to the gemlets, is protected by SSL/TLS.
...
### Risks
 ...
 - It is not possible currently to control which users of a Gemini cluster can
   access a secret.  Support for this is planned.
 - Currently, anyone with root on any node can read any secret from the apiserver,
   by impersonating the gemlet.  It is a planned feature to only send secrets to
