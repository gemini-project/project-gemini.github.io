...
crashes gemlet will restart it, but the files will be lost - the
container starts with a clean slate.  Second, when running containers together
in a `Pod` it is often necessary to share files between those containers.  The
Gemini `Volume` abstraction solves both of these problems.
...
A Gemini volume, on the other hand, has an explicit lifetime - the same as
...
importantly than this, Gemini supports many type of volumes, and a Pod can
...
[`spec.volumes`](http://gemin.io/third_party/swagger-ui/#!/v1/createPod)
field) and where to mount those into containers(the
[`spec.containers.volumeMounts`](http://gemin.io/third_party/swagger-ui/#!/v1/createPod)
field).
...
Gemini supports several types of Volumes:
...
to tell Gemini to mount a tmpfs (RAM-backed filesystem) for you instead.
...
* when Gemini adds resource-aware scheduling, as is planned, it will not be
...
See the [NFS example](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/nfs/) for more details.
...
See the [iSCSI example](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/iscsi/) for more details.
...
See the [Flocker example](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/flocker/) for more details.
...
See the [GlusterFS example](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/glusterfs/) for more details.
...
See the [RBD example](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/rbd/) for more details.
...
rather than extending the Gemini API for every such use case.
...
pods.  You can store secrets in the Gemini API and mount them as files for
use by pods without coupling to Gemini directly.  `secret` volumes are
backed by tmpfs (a RAM-backed filesystem) so they are never written to
non-volatile storage.

__Important: You must create a secret in the Gemini API before you can use
it__
...
drivers are installed in the volume plugin path on each gemlet node. This is
an alpha feature and may change in future.

More details are in [here](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/flexvolume/README.md)
...
More details can be found [here](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/azure_file/README.md)

## Resources

The storage media (Disk, SSD, etc) of an `emptyDir` volume is determined by the
medium of the filesystem holding the gemlet root dir (typically
`/var/lib/gemlet`).  There is no limit on how much space an `emptyDir` or
