...
So far the Pods we've seen have all used the `image` field to indicate what process Gemini
should run in a container.  In this case, Gemini runs the image's default command.  If we want
...
Gemini allows you to override both the image's default command (docker `Entrypoint`) and args
