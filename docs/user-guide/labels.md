...
If the prefix is omitted, the label key is presumed to be private to the user. Automated system components (e.g. `gem-scheduler`, `gem-controller-manager`, `gem-apiserver`, `gemctl`, or other third-party automation) which add labels to end-user objects must specify a prefix.  The `gemin.io/` prefix is reserved for Gemini core components.
...
Via a _label selector_, the client/user can identify a set of objects. The label selector is the core grouping primitive in Gemini.
...
Both label selector styles can be used to list or watch resources via a REST client. For example targeting `apiserver` with `gemctl` and using _equality-based_ one may write:

```shell
$ gemctl get pods -l environment=production,tier=frontend
```

or using _set-based_ requirements:

```shell
$ gemctl get pods -l 'environment in (production),tier in (frontend)'
```
...
```shell
$ gemctl get pods -l 'environment in (production, qa)'
```

or restricting negative matching via _exists_ operator:

```shell
$ gemctl get pods -l 'environment,environment notin (frontend)'
```

### Set references in API objects

Some Gemini objects, such as [`service`s](/docs/user-guide/services) and [`replicationcontroller`s](/docs/user-guide/replication-controller), also use label selectors to specify sets of other resources, such as [pods](/docs/user-guide/pods).
