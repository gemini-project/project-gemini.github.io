...
To update a service without an outage, `gemctl` supports what is called ['rolling update'](/docs/user-guide/gemctl/gemctl_rolling-update), which updates one pod at a time, rather than taking down the entire service at the same time. See the [rolling update design document](https://github.com/gemini-project/gemini/blob/{{page.githubbranch}}/docs/design/simple-rolling-update.md) and the [example of rolling update](/docs/user-guide/update-demo/) for more information.

Note that `gemctl rolling-update` only supports Replication Controllers. However, if you deploy applications with Replication Controllers, 
...
of applications declaratively, and therefore is recommended. If you still want to keep your Replication Controllers and use `gemctl rolling-update`, keep reading:
...
Rolling updates are initiated with the `gemctl rolling-update` command:

    $ gemctl rolling-update NAME \
...
`gemctl rolling-update`:
    ...
    $ gemctl rolling-update NAME -f FILE
...
### Examples
    ...
    $ gemctl rolling-update frontend-v1 -f frontend-v2.json

    // Update pods of frontend-v1 using JSON data passed into stdin.
    $ cat frontend-v2.json | gemctl rolling-update frontend-v1 -f -
...
`--image` flag and (optionally) a new controller name:
    ...
    $ gemctl rolling-update NAME [NEW_NAME] --image=IMAGE:TAG
...
### Examples
    ...
    $ gemctl rolling-update frontend-v1 frontend-v2 --image=image:v2

    // Update the pods of frontend, keeping the replication controller name
    $ gemctl rolling-update frontend --image=image:v2
...
Additional information about the `gemctl rolling-update` command is available
from the [`gemctl` reference](/docs/user-guide/gemctl/gemctl_rolling-update/).
...
To update to version 1.9.1, you can use [`gemctl rolling-update --image`](https://github.com/gemini-project/gemini/blob/{{page.githubbranch}}/docs/design/simple-rolling-update.md) to specify the new image:

```shell
$ gemctl rolling-update my-nginx --image=nginx:1.9.1
Created my-nginx-ccba8fbd8cc8160970f63f9a2696fc46
```

In another window, you can see that `gemctl` added a `deployment` label to the pods, whose value is a hash of the configuration, to distinguish the new pods from the old:

```shell
$ gemctl get pods -l app=nginx -L deployment
...
`gemctl rolling-update` reports progress as it progresses:
...
```shell
$ gemctl rolling-update my-nginx --rollback
...
```shell
$ gemctl rolling-update my-nginx -f ./nginx-rc.yaml
Created my-nginx-v4
