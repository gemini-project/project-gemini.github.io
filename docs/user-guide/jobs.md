...
To view completed pods of a job, use `gemctl get pods --show-all`.  The `--show-all` will show completed pods too.

To list all the pods that belong to job in a machine readable form, you can use a command like this:

```shell
$ pods=$(gemctl get pods --selector=app=pi --output=jsonpath={.items..metadata.name})
echo $pods
pi-aiw0a
```
...
A Job also needs a [`.spec` section](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/docs/devel/api-conventions.md#spec-and-status).
...
A job can be scaled up using the `gemctl scale` command.  For example, the following
command sets `.spec.parallelism` of a job called `myjob` to 10:

```shell
$ gemctl scale  --replicas=$N jobs/myjob
job "myjob" scaled
```
...
- One Job object for each work item, vs a single Job object for all work items.  The latter is
  ...
  (number of concurrently running pods) can be easily adjusted using the `gemctl scale` command.
...
|                            Pattern                                         | Single Job object | Fewer pods than work items? | Use app unmodified? |  Works in Gem 1.1? |
| -------------------------------------------------------------------------- |:-----------------:|:---------------------------:|:-------------------:|:-------------------:|
| [Job Template Expansion](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/job/expansions/README.md)          |                   |                             |          ✓          |          ✓          |
| [Queue with Pod Per Work Item](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/job/work-queue-1/README.md)  |         ✓         |                             |      sometimes      |          ✓          |
| [Queue with Variable Pod Count](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/job/work-queue-2/README.md) |                   |         ✓         |             ✓               |                     |          ✓          |
| Single Job with Static Work Assignment                                     |         ✓         |                             |          ✓          |                     |

When you specify completions with `.spec.completions`, each Pod created by the Job controller
has an identical [`spec`](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/docs/devel/api-conventions.md#spec-and-status).  This means that
...
| -------------------------------------------------------------------------- |:-------------------:|:--------------------:|
| [Job Template Expansion](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/job/expansions/README.md)          |          1          |     should be 1      |
| [Queue with Pod Per Work Item](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/job/work-queue-1/README.md)  |          W          |        any           |
| [Queue with Variable Pod Count](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/job/work-queue-2/README.md) |          1          |        any           |
...
in unpredicatable ways too.  Gemini will not stop you from making a mistake when
...
running, using `gemctl delete jobs/old-one --cascade=false`.
...
complicated to get started with and offers less integration with Gemini.

One example of this pattern would be a Job which starts a Pod which runs a script that in turn
starts a Spark master controller (see [spark example](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/examples/spark/README.md)), runs a spark
...
Support for creating Jobs at specified times/dates (i.e. cron) is expected in [1.3](https://github.com/gemini-project/gemini/pull/11980).
