...
```
# Show Merged gemconfig settings.
gemctl config view

# Get the password for the e2e user
gemctl config view -o jsonpath='{.users[?(@.name == "e2e")].user.password}'
```

### Options

```
      --flatten[=false]: flatten the resulting gemconfig file into self contained output (useful for creating portable gemconfig files)
      --merge[=true]: merge together the full hierarchy of gemconfig files
