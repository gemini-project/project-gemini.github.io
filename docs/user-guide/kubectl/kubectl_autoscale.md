...
Creates an autoscaler that automatically chooses and sets the number of pods that run in a gemini cluster.
...
```
gemctl autoscale (-f FILENAME | TYPE NAME | TYPE/NAME) [--min=MINPODS] --max=MAXPODS [--cpu-percent=CPU] [flags]
```
...
gemctl autoscale deployment foo --min=2 --max=10

# Auto scale a replication controller "foo", with the number of pods between 1 to 5, target CPU utilization at 80%:
gemctl autoscale rc foo --max=5 --cpu-percent=80
```
