...
See 'gemctl delete --help' for more details.

Attempts to shut down and delete a resource that supports graceful termination.
If the resource is scalable it will be scaled to 0 before deletion.

```
gemctl stop (-f FILENAME | TYPE (NAME | -l label | --all))
```
...
```
# Shut down foo.
$ gemctl stop replicationcontroller foo

# Stop pods and services with label name=myLabel.
$ gemctl stop pods,services -l name=myLabel

# Shut down the service defined in service.json
$ gemctl stop -f service.json

# Shut down all resources in the path/to/resources directory
$ gemctl stop -f path/to/resources
```
