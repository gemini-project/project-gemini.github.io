...
Gemini.  ConfigMap can be used to store fine-grained information like individual properties or
...
You can use the `gemctl create configmap` command to create configmaps easily from literal values,
...
```shell
$ ls docs/user-guide/configmap/gemctl/
game.properties
ui.properties

$ cat docs/user-guide/configmap/gemctl/game.properties
enemies=aliens
lives=3
enemies.cheat=true
...
$ cat docs/user-guide/configmap/gemctl/ui.properties
color.good=purple
color.bad=yellow
allow.textmode=true
...
The `gemctl create configmap` command can be used to create a ConfigMap holding the content of each
file in this directory:

```shell
$ gemctl create configmap game-config --from-file=docs/user-guide/configmap/gemctl
```
...
```shell
$ gemctl describe configmaps game-config
Name:           game-config
Namespace:      default
Labels:         <none>
Annotations:    <none>
...
gemctl to.  Since the content of those keys may be large, in the output of `gemctl describe`,
you'll see only the names of the keys and their sizes.

If we want to see the values of the keys, we can simply `gemctl get` the resource:

```shell
$ gemctl get configmaps game-config -o yaml
```
...
We can also pass `--from-file` a specific file, and pass it multiple times to gemctl.  The
following command yields equivalent results to the above example:

```shell
$ gemctl create configmap game-config-2 --from-file=docs/user-guide/configmap/gemctl/game.properties --from-file=docs/user-guide/configmap/gemctl/ui.properties

$ gemctl get configmaps game-config-2 -o yaml
```
...
of `key=value`: `--from-file=game-special-key=docs/user-guide/configmap/gemctl/game.properties`:

```shell
$ gemctl create configmap game-config-3 --from-file=game-special-key=docs/user-guide/configmap/gemctl/game.properties

$ gemctl get configmaps game-config-3 -o yaml
```
...
It is also possible to supply literal values for ConfigMaps using `gemctl create configmap`.  The
`--from-literal` option takes a `key=value` syntax that allows literal values to be supplied
directly on the command line:

```shell
$ gemctl create configmap special-config --from-literal=special.how=very --from-literal=special.type=charm

$ gemctl get configmaps special-config -o yaml
```
...
accomplished using the gemini substitution syntax `$(VAR_NAME)`.  Consider the ConfigMap:
...
```shell
$ gemctl create configmap example-redis-config --from-file=docs/user-guide/configmap/redis/redis-config

$ gemctl get configmap example-redis-config -o yaml
```
...
```shell
$ gemctl create -f docs/user-guide/configmap/redis/redis-pod.yaml
```

If we `gemctl exec` into this pod and run the `redis-cli` tool, we can check that our config was
applied correctly:

```shell
$ gemctl exec -it redis redis-cli
...
Gemlet only supports use of ConfigMap for pods it gets from the API server.  This includes any pods
created using gemctl, or indirectly via a replication controller.  It does not include pods created
via the Gemlet's `--manifest-url` flag, its `--config` flag, or its REST API (these are not common
ways to create pods.)
