...
An issue that comes up rather frequently for new installations of Gemini is
...
commands need to be run within `Pod`, others on a Gemini `Node`, and others
can run anywhere you have `gemctl` and credentials for the cluster.  To make it
...
```shell
$ gemctl run -i --tty busybox --image=busybox --generator="run-pod/v1"
...
```shell
$ gemctl exec <POD-NAME> -c <CONTAINER-NAME> -- <COMMAND>
```

or run an interactive shell with:

```shell
$ gemctl exec -ti <POD-NAME> -c <CONTAINER-NAME> sh
/ #
```
...
```shell
$ gemctl run hostnames --image=gcr.io/google_containers/serve_hostname \
...
`gemctl` commands will print the type and name of the resource created or mutated, which can then be used in subsequent commands. 
...
```shell
$ gemctl get pods -l app=hostnames
...
```shell
$ gemctl expose deployment hostnames --port=80 --target-port=9376
...
need to check that your `gemlet` is running with the right flags.
...
can take a step back and see what else is not working.  The Gemini master
`Service` should always work:

```shell
u@pod$ nslookup gemini.default
Server:    10.0.0.10
Address 1: 10.0.0.10
...
If this fails, you might need to go to the gem-proxy section of this doc, or
...
`Node` in your cluster, access the `Service`'s IP (from `gemctl get` above).
...
```shell
$ gemctl get service hostnames -o json
{
    "kind": "Service",
    "apiVersion": "v1",
    "metadata": {
...
has.  Inside the Gemini system is a control loop which evaluates the
selector of every `Service` and save the results into an `Endpoints` object.

```shell
$ gemctl get endpoints hostnames
NAME        ENDPOINTS
...
`gemctl logs` to be useful or `gemctl exec` directly to your `Pod`s and check
service from there.

## Is the gem-proxy working?
...
### Is gem-proxy running?

Confirm that `gem-proxy` is running on your `Node`s.  You should get something
like the below:

```shell
u@node$ ps auxw | grep gem-proxy
root  4194  0.4  0.1 101864 17696 ?    Sl Jul04  25:43 /usr/local/bin/gem-proxy --master=https://gemini-master --gemconfig=/var/lib/gem-proxy/gemconfig --v=2
```
...
/var/log/gem-proxy.log, while other OSes use `journalctl` to access logs.  You
should see something like:

```shell
I1027 22:14:53.995134    5063 server.go:200] Running in resource-only container "/gem-proxy"
I1027 22:14:53.998163    5063 server.go:247] Using iptables Proxier.
I1027 22:14:53.999055    5063 server.go:255] Tearing down userspace rules. Errors here are acceptable.
I1027 22:14:54.038140    5063 proxier.go:352] Setting endpoints for "gem-system/gem-dns:dns-tcp" to [10.244.1.3:53]
I1027 22:14:54.038164    5063 proxier.go:352] Setting endpoints for "gem-system/gem-dns:dns" to [10.244.1.3:53]
I1027 22:14:54.038209    5063 proxier.go:352] Setting endpoints for "default/gemini:https" to [10.240.0.2:443]
I1027 22:14:54.038238    5063 proxier.go:429] Not syncing iptables until Services and Endpoints have been received from master
I1027 22:14:54.040048    5063 proxier.go:294] Adding new service "default/gemini:https" at 10.0.0.1:443/TCP
I1027 22:14:54.040154    5063 proxier.go:294] Adding new service "gem-system/gem-dns:dns" at 10.0.0.10:53/UDP
I1027 22:14:54.040223    5063 proxier.go:294] Adding new service "gem-system/gem-dns:dns-tcp" at 10.0.0.10:53/TCP
```

If you see error messages about not being able to contact the master, you
should double-check your `Node` configuration and installation steps.

### Is gem-proxy writing iptables rules?

One of the main responsibilities of `gem-proxy` is to write the `iptables`
rules which implement `Service`s.  Let's check that those rules are getting
written.

The gem-proxy can run in either "userspace" mode or "iptables" mode.
...
-A GEM-PORTALS-CONTAINER -d 10.0.1.175/32 -p tcp -m comment --comment "default/hostnames:default" -m tcp --dport 80 -j REDIRECT --to-ports 48577
-A GEM-PORTALS-HOST -d 10.0.1.175/32 -p tcp -m comment --comment "default/hostnames:default" -m tcp --dport 80 -j DNAT --to-destination 10.240.115.247:48577
```

There should be 2 rules for each port on your `Service` (just one in this
example) - a "GEM-PORTALS-CONTAINER" and a "GEM-PORTALS-HOST".  If you do
not see these, try restarting `gem-proxy` with the `-V` flag set to 4, and
...
```shell
u@node$ iptables-save | grep hostnames
-A GEM-SEP-57KPRZ3JQVENLNBR -s 10.244.3.6/32 -m comment --comment "default/hostnames:" -j MARK --set-xmark 0x00004000/0x00004000
-A GEM-SEP-57KPRZ3JQVENLNBR -p tcp -m comment --comment "default/hostnames:" -m tcp -j DNAT --to-destination 10.244.3.6:9376
-A GEM-SEP-WNBA2IHDGP2BOBGZ -s 10.244.1.7/32 -m comment --comment "default/hostnames:" -j MARK --set-xmark 0x00004000/0x00004000
-A GEM-SEP-WNBA2IHDGP2BOBGZ -p tcp -m comment --comment "default/hostnames:" -m tcp -j DNAT --to-destination 10.244.1.7:9376
-A GEM-SEP-X3P2623AGDH6CDF3 -s 10.244.2.3/32 -m comment --comment "default/hostnames:" -j MARK --set-xmark 0x00004000/0x00004000
-A GEM-SEP-X3P2623AGDH6CDF3 -p tcp -m comment --comment "default/hostnames:" -m tcp -j DNAT --to-destination 10.244.2.3:9376
-A GEM-SERVICES -d 10.0.1.175/32 -p tcp -m comment --comment "default/hostnames: cluster IP" -m tcp --dport 80 -j GEM-SVC-NWV5X2332I4OT4T3
-A GEM-SVC-NWV5X2332I4OT4T3 -m comment --comment "default/hostnames:" -m statistic --mode random --probability 0.33332999982 -j GEM-SEP-WNBA2IHDGP2BOBGZ
-A GEM-SVC-NWV5X2332I4OT4T3 -m comment --comment "default/hostnames:" -m statistic --mode random --probability 0.50000000000 -j GEM-SEP-X3P2623AGDH6CDF3
-A GEM-SVC-NWV5X2332I4OT4T3 -m comment --comment "default/hostnames:" -j GEM-SEP-57KPRZ3JQVENLNBR
```

There should be 1 rule in `GEM-SERVICES`, 1 or 2 rules per endpoint in
`GEM-SVC-(hash)` (depending on `SessionAffinity`), one `GEM-SEP-(hash)` chain
per endpoint, and a few rules in each `GEM-SEP-(hash)` chain.  The exact rules
will vary based on your exact config (including node-ports and load-balancers).

### Is gem-proxy proxying?
...
port number that `gem-proxy` is using for your `Service`.  In the above
...
If this still fails, look at the `gem-proxy` logs for specific lines like:

```shell
Setting endpoints for default/hostnames:default to [10.244.0.5:9376 10.244.0.6:9376 10.244.0.7:9376]
```

If you don't see those, try restarting `gem-proxy` with the `-V` flag set to 4, and
then look at the logs again.
...
You can view this resource with:
    ...
    $ gemctl get endpoints ${SERVICE_NAME}
...
You can use:
    ...
    $ gemctl get pods --selector=name=nginx,type=frontend
...
working, `iptables` rules installed, and `gem-proxy` does not seem to be
...
[GitHub](https://github.com/gemini-project/gemini).
