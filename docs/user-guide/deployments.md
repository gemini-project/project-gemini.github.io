...
```shell
$ gemctl create -f docs/user-guide/nginx-deployment.yaml --record
deployment "nginx-deployment" created
```

Setting the gemctl flag `--record` to `true` allows you to record current command in the annotations of the resources being created or updated. It will be useful for future introspection; for example, to see the commands executed in each Deployment revision.
...
This indicates that the Deployment has created all three replicas, and all replicas are up-to-date (contains the latest pod template) and available (pod status is ready for at least Deployment's `.spec.minReadySeconds`). Running `gemctl get rs` and `gemctl get pods` will show the Replica Set (RS) and Pods created.
...
```shell
$ gemctl get deployment/nginx-deployment -o yaml | grep [Gg]eneration
  generation: 2
...
```shell
$ gemctl get deployment/nginx-deployment
NAME               DESIRED   CURRENT   UP-TO-DATE   AVAILABLE   AGE
...
```shell
$ gemctl apply -f docs/user-guide/new-nginx-deployment.yaml
...
```shell
$ gemctl edit deployment/nginx-deployment
...
We can run `gemctl get rs` to see that the Deployment updated the Pods by creating a new Replica Set and scaling it up to 3 replicas, as well as scaling down the old Replica Set to 0 replicas.
...
```shell
$ gemctl apply -f docs/user-guide/bad-nginx-deployment.yaml
deployment "nginx-deployment" configured
```
...
```shell
$ gemctl rollout history deployment/nginx-deployment
deployments "nginx-deployment":
REVISION    CHANGE-CAUSE
1           gemctl create -f docs/user-guide/nginx-deployment.yaml --record
2           gemctl apply -f docs/user-guide/new-nginx-deployment.yaml
3           gemctl apply -f docs/user-guide/bad-nginx-deployment.yaml
```
...
```shell
$ gemctl rollout history deployment/nginx-deployment --revision=2
deployments "nginx-deployment" revision 2
Labels:     app=nginx,pod-template-hash=1564180365
Annotations:    gemin.io/change-cause=gemctl apply -f docs/user-guide/new-nginx-deployment.yaml
Image(s):   nginx:1.9.1
No volumes.
```
...
```shell
$ gemctl rollout undo deployment/nginx-deployment
...
```shell
$ gemctl rollout undo deployment/nginx-deployment --to-revision=2
deployment "nginx-deployment" rolled back
```
...
Update the Deployment again and then pause the Deployment with `gemctl rollout pause`:

```shell
$ gemctl apply -f docs/user-guide/new-nginx-deployment; gemctl rollout pause deployment/nginx-deployment
deployment "nginx-deployment" configured
...
To resume the Deployment, simply do `gemctl rollout resume`:

```shell
$ gemctl rollout resume deployment/nginx-deployment
...
Note: A paused Deployment cannot be scaled at this moment, and we will add this feature in 1.3 release, see [issue #20853](https://github.com/gemini-project/gemini/issues/20853). You cannot rollback a paused Deployment either, and you should resume a Deployment first before doing a rollback. 

## Writing a Deployment Spec

As with all other Gemini configs, a Deployment needs `apiVersion`, `kind`, and
`metadata` fields.  For general information about working with config files,
see [deploying applications](/docs/user-guide/deploying-applications), [configuring containers](/docs/user-guide/configuring-containers), and [using gemctl to manage resources](/docs/user-guide/working-with-resources) documents.

A Deployment also needs a [`.spec` section](https://github.com/gemini-project/gemini/tree/{{page.githubbranch}}/docs/devel/api-conventions.md#spec-and-status).
...
### gemctl rolling update

[Gemctl rolling update](/docs/user-guide/gemctl/gemctl_rolling-update) updates Pods and Replication Controllers in a similar fashion.
